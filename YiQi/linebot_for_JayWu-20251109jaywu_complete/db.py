...
def user_exists(line_user_id):
    ...
    with conn:
        with conn.cursor() as cursor:
            sql = "SELECT * FROM users_yichi WHERE line_user_id = %s"
...
def add_user(line_user_id):
    ...
    with conn:
        with conn.cursor() as cursor:
            # 先查詢最大的 id 值
            cursor.execute("SELECT MAX(id) as max_id FROM users_yichi")
            result = cursor.fetchone()
            next_id = 1 if result['max_id'] is None else result['max_id'] + 1
            
            # 插入新用戶時指定 id
            sql = "INSERT INTO users_yichi (id, line_user_id) VALUES (%s, %s)"
...
def update_user_step(line_user_id, step):
    ...
    with conn:
        with conn.cursor() as cursor:
            sql = "UPDATE users_yichi SET step = %s WHERE line_user_id = %s"
...
def update_user_field(line_user_id, field_name, value):
    ...
    with conn:
        with conn.cursor() as cursor:
            sql = f"UPDATE users_yichi SET {field_name} = %s WHERE line_user_id = %s"
...
def update_user_mode(line_user_id, mode):
    ...
    with conn:
        with conn.cursor() as cursor:
            sql = "UPDATE users_yichi SET mode = %s WHERE line_user_id = %s"
...
def update_temp_value(line_user_id, value):
    ...
    with conn:
        with conn.cursor() as cursor:
            sql = "UPDATE users_yichi SET temp_value = %s WHERE line_user_id = %s"
            cursor.execute(sql, (value, line_user_id))
            conn.commit()
...
def append_address(line_user_id, new_address):
    ...
    with conn:
        with conn.cursor() as cursor:
            # 取得現有的戶別字串
            sql = "SELECT address FROM users_yichi WHERE line_user_id = %s"
            cursor.execute(sql, (line_user_id,))
            result = cursor.fetchone()
            ...
            # 更新資料庫
            sql = "UPDATE users_yichi SET address = %s WHERE line_user_id = %s"
    ...
    with conn:
        with conn.cursor() as cursor:
            try:
                ...
                WHERE TABLE_SCHEMA = %s AND TABLE_NAME = 'users_yichi' AND COLUMN_NAME = %s
                """
